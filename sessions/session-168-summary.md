...
- **Process — CI was red since S167:** GitHub CI failed on PR #202 (and on S167's PRs #200 and #201, which merged anyway): `actions/checkout@v4` clones depth 1, so the tests that read shipped demo templates out of git history (`a_shipped_unstamped_demo_template_upgrades_and_the_list_matches_git_history`, S167; `s167_stamped_kit_and_template_upgrade_as_stale_renders`, S168) found `.git` but not the commits. Fixed with `fetch-depth: 0` in `.github/workflows/ci.yml`; the review SHA was re-attested for that config-only change.
