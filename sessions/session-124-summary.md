...
| 2 | `vajra next --dogfood-age` recorded pre/post-run; post-run shows S124 as most recent | SHIPPED | `pre-run-baseline.txt` recorded pre-run (S118, stale). Immediately post-run it still showed S118 — `--dogfood-age` (`src/dogfood/mod.rs`) does not recurse into artifact subdirectories (a named, pre-existing weakness, S115), and the harness had written `receipt.stderr.txt` only under `p1/`, not the artifacts-dir top level S118's own convention used. Fixed by mirroring `receipt.stderr.txt` + `run-result.json` to the top level (not a `src/` change — an artifact-layout fix matching established convention). Re-verified after commit: `vajra next --dogfood-age` now reports `last dogfood session: 124`, cost `$3.2985`, `0 sessions since`. |
...
**Count: 7 of 9 SHIPPED, 2 PARTIAL, 0 NOT-BUILT.**
