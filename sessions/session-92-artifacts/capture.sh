#!/usr/bin/env bash
# S92 dogfood ride-along — capture harness (adapted from S76).
#
# Instruments ONE headless `vajra claude -p` run over the chitra subject repo and
# preserves every raw surface. No src changes; no fixing bugs mid-run (S63 stance).
#
...
#
# Usage:
#   sessions/session-92-artifacts/capture.sh -f path/to/prompt.txt
#   sessions/session-92-artifacts/capture.sh "the one real task prompt"
set -uo pipefail

ART="/Users/suman/playground/vajra/sessions/session-92-artifacts"
CH="/Users/suman/playground/chitra"
CHSLUG="-Users-suman-playground-chitra"
PROJ="$HOME/.claude/projects/$CHSLUG"
# Pin the CURRENT release binary (installed ~/.cargo/bin/vajra was rebuilt this session too).
