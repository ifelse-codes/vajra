...
- The terminal demo IS the human demo (DECISION-009): the demo script, drawn with \
`scripts/demo-kit.sh`, plays as a slide deck in a terminal. Never propose a separate HTML deck.\n\
- Propose content for EVERY section of the outline in `scripts/demo-session-template.sh`: \
headline (which session, what it delivered, `dk_vajra_tiles NN` plus any extra tiles each marked \
live or recorded, the change in one breath) · story (what happened, in plain words) · \
before_after · rule (the new rule or behaviour in plain words) · cases (ordinary and odd inputs) \
· scorecard (`dk_vajra_scorecard NN`, live checks vs recorded numbers, honest notes, what the demo \
does not show) · next (the next options plus a small-words helper for every term a newcomer would \
trip on).\n\
- Four sections carry the four scanned elements — name them: headline → `demo:header`, \
before_after → `demo:before_after`, cases → `demo:cases`, scorecard → `demo:summary_table`. The \
kit's `dk_section` prints each marker where its section renders; an unfilled `dk_todo` fails the \
demo by name.\n\
- A demo that cannot be faked (DECISION-010): the numbers Vajra knows — stations passed, the \
review verdict, recommendations answered, crew handoffs — are FILLED IN BY VAJRA with \
`dk_vajra_tiles NN` and `dk_vajra_scorecard NN` (from `vajra next --demo-facts NN`). Never propose a \
hand-typed tile for any of them: the gate re-derives every `demo:fact` at close and blocks a \
mismatch.\n\
- Propose every live check as a COMMAND: `dk_check \"label\" <command…>`, whose real exit code \
decides (name the command). A bare `PASS`, `FAIL` or digit is refused by the kit, and the gate \
requires `demo:complete`, which only a passing `dk_finish` prints.\n\
...
pub const ROLES: &[Role] = &[
    ...
    Role {
        name: "demo-producer",
        description: "Propose every section of a session's terminal demo deck — headline, story, \
                      before-and-after, rule, cases, scorecard, next — so the gate's live re-run \
                      proves what shipped. Use before the demo is written. Read-only.",
