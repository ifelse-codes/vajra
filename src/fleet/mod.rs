...
headline (which session, what it delivered, number tiles each marked live or recorded, the change \
in one breath) · story (what happened, in plain words) · before_after · rule (the new rule or \
behaviour in plain words) · cases (ordinary and odd inputs) · scorecard (live checks vs recorded \
numbers, honest notes, what the demo does not show) · next (the next options plus a small-words \
helper for every term a newcomer would trip on).\n\
