...
#
# ONE SOURCE (S123 fix): `print_tally()`/`tally_discloses_nesting()` were byte-duplicated across
# this file and verify-session-121.sh with nothing binding the copies — S122 fixed exactly this
# drift-by-copy hazard for the execution policy (`execution_policy_one_source`, below) and created
# a fresh instance of it for the tally in the same diff. Both now source the one definition
# (`scripts/lib-tally.sh`, sourced near the top of this file).
...
# The tally is ONE SOURCE (S123 fix) — same check as verify-session-121.sh, run independently in
# THIS process since each script is its own bash invocation.
tally_is_one_source() {
  shopt -s extdebug
  local pt tn
...
# `tally_discloses_nesting()` is the ONE definition sourced from `lib-tally.sh` above (S123 fix) —
# no second, possibly-looser copy to police this suite's own tally with.
own_tally_has_teeth() {
  local rc=0 OLD NEW
