...
#        DEMO_AUTOPLAY=seconds per slide · NO_COLOR=1.
#
...
# no live check ran, or a live check failed.
#
...
if [ "$DK_COLOR" = 1 ]; then
  case "${COLORTERM:-}" in   # one accent (violet), green/red only for pass/fail, dim for chrome
...
DK_SECTIONS="" DK_TODOS="" _DK_UNKNOWN=""
...
dk_check() { local res="$2"; case "$res" in 0|PASS) res=PASS ;; *) res=FAIL ;; esac   # "label" rc|PASS|FAIL [quiet]
  DK_SCORES+=("$res|$1"); [ "$res" = FAIL ] && DK_FAILS=$((DK_FAILS+1))
  [ "${3:-}" = quiet ] && return 0
  if [ "$res" = PASS ]; then printf '  %s\n' "${C_YES}✓${C_0} $1"; else printf '  %s\n' "${C_NO}✗ $1${C_0}"; fi; }
