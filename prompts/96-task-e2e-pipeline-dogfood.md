# Session 96 — Dogfood: end-to-end 8-station pipeline on chitra

> **Status:** APPROVED (founder pick A, S95 GT close).
...
3. **Receipt** — `sessions/session-96-artifacts/receipt.stderr.txt` + `run-result.json` with
   `total_cost_usd` (the S78 tee path, `vajra claude -p`).
4. **Dogfood age** — `vajra next --dogfood-age` after the run; should now show **S96** most recent.
...
Receipt artefacts save to **Vajra's** `sessions/session-96-artifacts/`, not chitra's.
...
2. `sessions/session-96-artifacts/receipt.stderr.txt` + `run-result.json` exist and are committed. `covers: 1`
...
7. `sessions/session-96-summary.md` (dogfood report) exists with verdict rows for: cost, stations
...
3. Capture receipt artefacts into `sessions/session-96-artifacts/`. Confirm `total_cost_usd`. `covers: 1, 2`
...
6. Document obedience; write `sessions/session-96-summary.md`. `covers: 6, all`
...
- **DOGFOOD session:** `VAJRA_CLOSEOUT_WAIVER=dogfood-no-src-changes` — no Vajra `## Execution` shas
  for this (Vajra S96) session. The chitra S08 sub-session follows its OWN closeout rules (and its
  Coder shas ARE the measurement — do not waive chitra's).
- No fidelity review required for Vajra S96 (DOGFOOD, no `src/` deliverable).
- Vajra branch: `session-96-e2e-pipeline-dogfood`. **New chat.**
...
- Artefacts go in **Vajra's** `sessions/session-96-artifacts/`, not chitra's.

## Delta (Analyst gate)

- `+` `sessions/session-96-artifacts/` — receipt + run-result.json (evidence)
- `+` `sessions/session-96-summary.md` — measured end-to-end dogfood report
