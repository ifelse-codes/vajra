...
| S172 | **Next — founder picks** | Three candidates in `sessions/session-171-summary.md`: (1) make the crew dispatch HAPPEN rather than be advised — F31, the last open finding from the founder's run; (2) finish the 0.2.0 release (crates.io still 0.1.0, so nobody outside this machine has any of S171); (3) keep testing rudra and fix what that finds. **Carried in from S171's pass-3 review:** rec 7 — an executable test for the pre-commit/pre-push human-vs-agent split (the riskiest change of S171, run by hand only). |
