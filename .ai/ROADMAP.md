...
| Last closed session | Session 128 — **CODE: first contact works — ACCEPT** (one cold pass; 14 SHIPPED · 2 PARTIAL · 2 NOT-BUILT · 1 N/A). The first user-reachable change since S108. `vajra --version` exists, the front door fails CLOSED (exit 2), `verify-closeout.sh` survives bash 3.2, `vajra check` is 10/11 on a fresh init, and `stranger_check` became a required GT audit. **Its residual was this session's story: the scaffold was still a fork.** |
| Prior session | Session 125 — **NO-CODE GT + FULL-STACK REVIEW — PARTIAL PASS.** The loop is closed: 16 sessions with no user-reachable change; 0 stars after 55 days public. Findings PARKED by founder call — **S128 unparked the first-contact slice (F3/F5) and the rest stays parked.** |
...
| Active session | **Session 129 — CODE: one source for what a stranger gets — founder pick A.** The scaffold is no longer a hand-typed fork. `build.rs` derives the scaffold's Hard Rules table and `required_audits` from the live `.ai/` at compile time: a stranger now receives **13 of 13** binding rules (was 8, two of them renamed) and **10 of 12** ground-truth audits (was 7), with the two withheld ones — `stranger_check`, `scaffold_drift_check` — named **in their own file, with the reason**. The DEFAULT is CARRIED, so a rule added here reaches every future scaffold with no action taken; a stale declaration fails the BUILD. New guard `scripts/scaffold-drift.sh` (12/12) compares the LIVE `.ai/` against what a REAL `vajra init` writes into a REAL empty directory, and NAMES what it compared. verify **11/11** · demo **13/13** · stranger **20/20** (was 16) · fixture **13/13** (5 plants + a control) · **364** tests · `K of 8` unmoved · **7 commands**. **Residual, unsoftened: only two lists are derived**; the rest of the scaffold's constitution is still hand-written prose, carrying a rule is not enforcing it, and 0 stars / 19 downloads are unchanged. |
...
- **F1 — the scaffold IS the constitution** — **[x] DONE at S129** (founder pick A). The binding
  ...
  `.ai/CONSTRAINTS.yaml#ground_truth`: **13 of 13** binding rules and **10 of 12** audits reach a
  stranger, the two withheld audits ship their reason in the stranger's own file, the DEFAULT for
  anything new is CARRIED, and a stale declaration fails the BUILD. `scripts/scaffold-drift.sh` is
  the execute-based guard (12/12) and `scaffold_drift_check` is the 12th required GT audit.
  **Still open, named not softened:** only TWO lists are derived — the rest of the scaffold's
  constitution is hand-written prose, and any OTHER list here may have a scaffolded twin. **What it
...
- **A1 — one outside user** *(no code, has lead time)*: 0 stars / 0 issues / 19 downloads after 57
  days public — **unchanged by S128**, which fixed the front door and moved no adoption number is the only metric that has never moved. Founder call recorded: keep the S118
