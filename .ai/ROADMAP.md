...
reclassified. Attested `c92a2dad…`. Founder pick: **S122 = prove the dispatch and take the first
LIVE QA run.** Summary: `sessions/session-121-summary.md`.
...
| Active session | None — between sessions (S121 complete). **S122 = CODE: prove `subagent_type: "qa-specialist"` dispatches by name + take the FIRST LIVE QA run** (founder pick A at the S121 close). Prompt: `prompts/122-task-qa-specialist-dispatch.md`. 🔒 Founder directive S118: README/VISION claims are the target spec, never softened to match reality; **no release until reality meets them.** |
...
| S122 | **Next (approved)** | **CODE: prove the QA Specialist dispatches by name + the FIRST LIVE QA run** (`prompts/122-task-qa-specialist-dispatch.md`) — founder pick A at the S121 close. Same S114→S115 / S116→S117 pattern, plus the part those sessions had no analogue for: hand the executing agent a REAL verify script and find out whether an executor produces evidence a reader could not. **A flat, agreeable report is a REAL finding** — to be recorded, never softened. |
