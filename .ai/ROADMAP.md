...
|---|---|
| Today | 2026-07-04 |
| Current phase | **S41 done — compression fail-gate fixed correctness-first** (git family folds regardless of `exitCode`; generic path stays conservative; failures never hidden). Enforcement moat unchanged from S40: *correct + scaffolded* but *live-unverified* (dogfood gate UNMEASURED). Next = S42 (git-level hooks + `jq`-preflight, founder pick C), then a standing live re-dogfood (#17a). |
| Last closed session | Session 41 — compression fail-gate fix (CODE, founder pick B); the git family now folds on real CC; `verify-session-41.sh` 20/20; `sessions/session-41-summary.md` |
| Active session | None — S41 closed; S42 (git-level hooks + jq, CODE) not yet started (new chat) |
...
17. **[ ] Git-level `pre-push`/`pre-commit` scaffolding + `jq`-preflight (S42 — FOUNDER PICK C, NEXT)** — scaffold a tracked `.githooks/pre-push` + `pre-commit` + `git config core.hooksPath` as a belt-and-suspenders L2 layer on scaffolded projects (closes the raw-`echo > .ai/SESSION` bypass); **bundle the S40 `jq`-preflight / fail-closed fix** (AGENTS.md L147 "a check that cannot evaluate FAILS" is violated by the hooks' `jq`-missing → exit 0 fail-open). Ready: `prompts/42-task-git-level-hooks-jq-preflight.md`. Branch `session-42-git-level-hooks-jq-preflight`. **Recommended split at BOOT: Gap 1 (jq-preflight) first, Gap 2 (git-level scaffold) second — carry to S43 if both won't fit one clean session.**
