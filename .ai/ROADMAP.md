...
**Updated:** 2026-07-03 · **Session 37 — Close the enforcement leak (CODE) — DONE.** Shipped `scripts/hook-publish-guard.sh` (PreToolUse Bash): the outward/irreversible actions the S36 agent took unstopped — `git push` / `gh pr create` / `gh pr merge` / `glab mr *` — now **BLOCK at L2/L3 (exit 2)** unless the founder set `VAJRA_ALLOW_PUBLISH=1` at launch; L1 advises; innocuous git (`status`/`log`/`diff --stat`) passes. Approval = env var (not a token file — the agent can `touch` a file but cannot mutate the hook's launch env). **Proved live:** the guard blocked the agent's own `git push` tool call. `verify-session-37.sh` 22/22 green; no `src/` change; 3 files. **Repo-only** — `vajra init` doesn't scaffold the guard yet, and that is where S36 leaked. Report: `sessions/session-37-summary.md`. **Next = S38 (founder pick A): propagate the guard into `vajra init` + git-level `pre-push`** (`prompts/38-task-propagate-publish-guard.md`). Also ready: compression fail-gate (`prompts/39-task-fix-compression-exit-gate.md`); session-boundary hardening (the other S36 slice).
...
|---|---|
| Today | 2026-07-03 |
| Current phase | **Closing the S36 enforcement leak.** S37 shipped the publish-guard (repo-only). Next = propagate it into `vajra init` + git-level hooks (S38) so scaffolded projects — the actual S36 leak site — are covered too |
| Last closed session | Session 37 — publish-guard: `git push`/`gh pr create`/`gh pr merge` now block at L2/L3 unless `VAJRA_ALLOW_PUBLISH=1`; proved live (blocked the agent's own push); 22/22 green; repo-only |
| Active session | None — S37 closed; S38 (propagate the guard, founder pick A) not yet started |
...
| `vajra init` command | [x] done — scaffolds `.ai/` + hooks (in `.ai/hooks/`, S34) + pointers + Darshan skill (20 files greenfield / 21 brownfield incl. the session-0 brief, interactive, idempotent) |
...
13. **[ ] Propagate the guard into `vajra init` + git-level hooks (S38, recommended #1 — founder pick)** — `vajra init` doesn't scaffold `hook-publish-guard.sh`, so scaffolded projects (the actual S36 leak site) stay unguarded. Scaffold the guard via the S29 `include_str!` one-source pattern (+ Cargo un-exclude + `TPL_CLAUDE_SETTINGS` wiring + tests) and add a git-level `pre-push`. `prompts/38-task-propagate-publish-guard.md`.
14. **[ ] Harden the session boundary (S39 candidate — the other S36 slice)** — arm the guard on *any* session advance (`SESSION` edit / `vajra next --advance`), not just `git checkout -b`, so the brownfield 00→01 jump can't blow the boundary unstopped. Design-bearing (define the "advance" signal). No prompt yet.
15. **[ ] Fix the compression fail-gate, correctness-first (S39/later)** — unblock the safe format-aware folds (git\*) regardless of `exitCode`; keep the generic path conservative; **never hide a failure** (founder directive). Proven defect, but the quiet bonus — behind the enforcement work. `prompts/39-task-fix-compression-exit-gate.md`.
16. **[ ] Trim the boot-packet cost** — the heavy constitution drove ~$32 cache-read of a $58 session; move toward the "<5% footprint" rule now that compression is shown to save ~$0. No prompt yet.
