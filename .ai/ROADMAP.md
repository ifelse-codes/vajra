...
| S128 | **Next** | **CODE: first contact works — fix what a stranger actually hits (founder pick C at the S127 closeout).** Four defects, all re-confirmed LIVE in an empty directory with the release binary: `vajra --version` does not exist (prints help, exit 0) · an unknown subcommand exits **0**, so `vajra <typo> && deploy` **runs deploy** — the front door fails OPEN · `vajra check` on a fresh init is **9/11**, incl. `vajra.varta missing`, a file `init` never creates · `scripts/verify-closeout.sh` **crashes** on a fresh repo (`set -u` + empty glob, bash 3.2 = macOS default) — the L4 fail-closed layer broken on first contact. Plus a **`stranger_check`** added to `ground_truth.required_audits`, because this class hid for 125 sessions while every instrument measured Vajra governing itself. **Candidate B was REJECTED ON PRINCIPLE by the founder** — an agent that reads advice and reports it did something it did not is a truthfulness problem, and a mechanical guardrail is the wrong tool for it. Candidate A was set aside as extending a team nobody outside this repo can use yet. **UNPARKS the first-contact slice of the S125 reboot backlog; the rest stays parked, including the scaffolded constitution fork (66 lines vs 183) which this session must NOT widen into.** Prompt: `prompts/128-task-first-contact-works.md` (DRAFT). |
