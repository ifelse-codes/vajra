...
| S129 | **Next** | **Founder pick pending** — three ranked candidates in `sessions/session-128-summary.md`. **A (recommended): one source for what a stranger gets** — kill the scaffold fork (66-line constitution vs 183 **and** 7-entry `required_audits` vs 11), derived, with a check that fails when the two drift; this is backlog **F1**, grown by S128's second finding. *Risk:* the two files are not supposed to be identical, so "derive" may mean writing a real derivation rather than an `include_str!`. **B: decide the first-contact exit codes and unblock `vajra init`** — is a fresh `vajra check` exiting 1 right (`branch: not main` on a fresh `git init`)? Make `init` work without a TTY. *Risk:* footnote-sized unless the exit-code question is genuinely answered. **C: a paid dogfood ride-along through the new front door**, from a freshly scaffolded repo. *Risk:* spends money for findings not fixes, and S130's GT runs `stranger_check` anyway. Draft prompt scaffolded for A: `prompts/129-task-one-source-scaffold.md` (**DRAFT** — the Analyst gate blocks the advance until the founder flips it). |
