...
| Active session | **Session 127 — CODE: every recommendation must be ANSWERED — ACCEPT (pass 2; pass 1 REJECTed and was right).** The first gate that CONSUMES a governed handoff: advice a session asked for must carry a recorded, existence-gated disposition or the close BLOCKS. Forces an ANSWER, never obedience. Dogfooded on itself — 51 recommendations from 3 roles, all answered. **Residual: four `obeyed:` labels in that ledger were wrong and passed; only readers caught them, and one gate of eight consumes handoffs.** Report: `sessions/session-127-summary.md`; review: `sessions/session-127-review.md`. |
