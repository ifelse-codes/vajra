...
| Active session | **Session 129 — CODE: one source for what a stranger gets — founder pick A.** The scaffold is no longer a hand-typed fork. `build.rs` derives the scaffold's Hard Rules table and `required_audits` from the live `.ai/` at compile time: a stranger now receives **13 of 13** binding rules (was 8, two of them renamed) and **10 of 12** ground-truth audits (was 7), with the two withheld ones — `stranger_check`, `scaffold_drift_check` — named **in their own file, with the reason**. The DEFAULT is CARRIED, so a rule added here reaches every future scaffold with no action taken; a stale declaration fails the BUILD. New guard `scripts/scaffold-drift.sh` (12/12) compares the LIVE `.ai/` against what a REAL `vajra init` writes into a REAL empty directory, and NAMES what it compared. New guard `scripts/scaffold-drift.sh` (**17/17**) compares the LIVE `.ai/` against what a REAL `vajra init` writes into a REAL empty directory, and its GREEN line now states its own JURISDICTION. **TWO cold passes, both ACCEPT** — pass 1 found a THIRD fork (`drift_axes`, 6 vs 7, three lines above the fix; now derived), pass 2 found a FOURTH (`TPL_CONSTRAINTS` hand-types a family of twins; **two already drifted**, one read by the product) which was **REFUSED with a reason** and is the S131 pick. **17 recommendations, 16 obeyed, 1 refused.** Unplanned find: `vajra next --check-plan` had been mis-parsing EVERY prompt since the heading changed — a `K of 8` station reported PASSED off it. verify **12/12** · demo **15/15** · drift **17/17** · stranger **21/21** (was 16) · fixture **18/18** (7 plants + a control) · **365** tests · `K of 8` unmoved · **7 commands**. |
