...
| S123 | **Next (approved)** | **CODE: fence the `Write`/`Edit` grant** (`prompts/123-task-fence-the-write-grant.md`) — founder option A of three at the S122 close. Make it structurally impossible for the QA role to edit the code it tests, instead of asking it not to; on both live runs the tree was unchanged only because the agent chose to hold the line. Steps 1–2 clear S122's own debt first (the two glued-on teeth, the byte-duplicated `print_tally`). **Design-significant: YES** — narrow the grant vs extend the L3 `hook-pre-write.sh` surface, decided in a `DECISION-007` S123 addendum before code lands. Non-goal: this does NOT prove the executor thesis. |
| S124 | Open | Unassigned. |
