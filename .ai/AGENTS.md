...
4. **EXECUTE** — Atomic changes. Update `ROADMAP.md` [x] on completion. Update `KNOWLEDGE.md` on new permanent fact. **CODE sessions:** record each plan step's commits in the prompt's `## Execution` section (`step N — done: <sha>`) as work lands — not just at close. `verify-closeout.sh` blocks closing when real plan steps exist but `## Execution` is absent or any `done: <sha>` placeholder remains unfilled (S154).
5. **VERIFY + DEMO** — `scripts/verify-session-NN.sh` exits 0 = done. `scripts/demo-session-NN.sh` shows what was built (cumulative — includes prior session capabilities). Artifacts at `.ai/verify/session-NN/<ts>/` with `latest` symlink. **The terminal demo IS the human demo (DECISION-009):** copy `scripts/demo-session-template.sh` (the seven-section outline, drawn with `scripts/demo-kit.sh`) and fill every section with live runs — in a terminal it plays as a slide deck; piped it prints every slide plus the `demo:` markers the gate scans. When the user asks to see the demo, run that script — never build a separate HTML deck.
