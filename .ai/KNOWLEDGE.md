...
## S167 — permanent facts

- **Reusing a session number whose old merged branch still exists makes the Releaser pass early.** `vajra next --stations 167` read `[PASSED] Releaser SHIP` before any S167 work, off the merged `origin/session-167-adhoc-fixes`. Prune the old branch (founder approval) or `git fetch --prune` before trusting a `session-NN-*` ship check.
- **A cold review cannot be filed retroactively for a closed session.** `vajra next --role` stamps the handoff with the CURRENT branch's number, and the fidelity gate re-verifies the subagent transcript's `gitBranch` against `session-NN-*`. A review of S166 run from S167's branch is provably not S166's; the crew gate (tech-lead) has no override at all. Close every session with its own tech-lead + fidelity handoffs — they cannot be made up later.
...
- **`vajra next --advance` asks `[y/N]` only at a real terminal** (S173 F52). With no terminal (an agent's shell, CI, a pipe) it advances and says it did not ask — piping `echo y` is no longer needed or meaningful. In a chat, the session guard is the control, not the question.
