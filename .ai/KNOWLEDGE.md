...
- **`vajra next --advance` asks `[y/N]` only at a real terminal** (S173 F52). With no terminal (an agent's shell, a pipe) it advances and says nobody was asked — piping `echo y` is no longer needed or meaningful.
