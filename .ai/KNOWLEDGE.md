...
- 2026-06-27 Session 19: **Varta v0 shipped** as a skill (not a compiler) in `varta/`. `SKILL.md` teaches the 9-construct ⚡ grammar + boot ritual (read→internalize→speak), modeled on the plain-talk skill. `GRAMMAR.varta` is the canonical spec, written in Varta itself (dogfood). `vajra.varta` renders Vajra's `.ai/` operating context in Varta as a **companion** (markdown stays source of truth). `READBACK.md` is the proof — 6/6 rules answerable from the spec alone. Grammar frozen at 9 constructs: `⚡project ⚡forbid ⚡require ⚡max ⚡pipeline ⚡final ⚡on ⚡assert ⚡enum`; anything that doesn't fit goes in a `//` comment. Decisions: standalone files (init-wiring deferred), companion-not-replacement. `scripts/verify-session-19.sh` checks structural invariants (no LLM in CI). S21 = the co-pilot loader (make `⚡on` fire at runtime).
