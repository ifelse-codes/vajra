...
- After upgrading `vajra` and running `vajra init --sync-fleet`, it gets the new kit and template.
  ...
  naming the downgrade — until the project adds `complete` to its `required_elements`.
...
- A hand-typed `echo demo:complete` still satisfies the `complete` element; only the facts check
  is unforgeable.
